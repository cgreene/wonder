---
name: orcid
description: Use to set, check, or read the user's ORCID iD for /wonder — a public researcher identifier stored locally and added (opt-in) to the wonder profile. Invoke when the user wants to set/change/clear their ORCID, or when /wonder needs the user's identity.
---
...
1. **First run / on request:** if no iD is set, offer to set one ("paste your ORCID
   so people know who you are"). It's **opt-in** — `/wonder` works fine without it.
...
- [ ] Wire into `commands/wonder.md` + `summarize-session` (read config → set `orcid` field, exempt it from scrub-pii).
- [ ] Send `orcid` to the OhWow server (`connect`) so it can attribute the room intro.
