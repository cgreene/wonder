...
# requires-python = ">=3.9"
# ///
"""Manage the user's ORCID iD for /wonder.

The iD is stored locally (~/.wonder/orcid by default; $WONDER_ORCID overrides the
location). It is OPT-IN: /wonder includes it in the profile only if it is set, and
...
def orcid_path() -> Path:
    return Path(os.environ.get("WONDER_ORCID", "") or Path.home() / ".wonder" / "orcid")
