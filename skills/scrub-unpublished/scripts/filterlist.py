...
appear in a wonder profile (compound names, project codenames, collaborator
...
default ~/.wonder/never-share.txt, overridable via $WONDER_FILTER_LIST.
...
def filterlist_path() -> Path:
    return Path(
        os.environ.get("WONDER_FILTER_LIST", "")
        or Path.home() / ".wonder" / "never-share.txt"
    )
